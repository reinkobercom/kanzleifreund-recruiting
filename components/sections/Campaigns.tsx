    ...
    <section className="bg-white px-6 md:px-20 py-20">
      <div className="max-w-5xl mx-auto">
        ...
        <h2 className="font-serif text-3xl md:text-5xl text-navy mb-3">
          ...
          <span className="text-navy/65">Nicht da, wo sie suchen.</span>
        </h2>
        <div className="w-12 h-[2px] bg-gold mb-10" />

        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div>
            <p className="text-navy/55 text-xs tracking-[0.15em] uppercase mb-6">Wie eine Kampagne abläuft</p>
            {steps.map((s, i) => (
              <div key={s.num} className={`flex gap-5 py-5 ${i < steps.length - 1 ? 'border-b border-navy/8' : ''}`}>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span className="text-[10px] text-navy/50 italic">· {s.period}</span>
                  </div>
