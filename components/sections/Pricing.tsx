  ...
  return (
    <section className="bg-offwhite px-6 md:px-20 py-20">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-navy/65 text-sm mb-12">Klare Pakete, klare Preise. Keine Erfolgshonorare. Alle Preise zzgl. MwSt.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={p.label} className={`p-6 rounded-sm ${p.highlight ? 'bg-navy' : 'bg-white border border-navy/8'}`}>
                <p className={`text-[10px] tracking-[0.2em] uppercase mb-3 ${p.highlight ? 'text-gold' : 'text-navy/55'}`}>{p.label}</p>
                <div className="flex items-baseline gap-1 mb-1">
                  <span className={`font-serif text-4xl ${p.highlight ? 'text-white' : 'text-navy'}`}>{p.price} €</span>
                  <span className={`text-xs ${p.highlight ? 'text-white/55' : 'text-navy/50'}`}>/ Monat</span>
                </div>
                <p className={`text-xs mb-4 ${p.highlight ? 'text-white/65' : 'text-navy/55'}`}>{p.stellen}</p>
                <div className={`border-t pt-3 ${p.highlight ? 'border-white/20' : 'border-navy/8'}`}>
                  <p className={`text-xs leading-relaxed ${p.highlight ? 'text-white/65' : 'text-navy/55'}`}>{p.features}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mb-10">
          <div className="flex items-baseline justify-between mb-4">
            ...
            <span className="text-[10px] text-navy/55 tracking-wide">pro 4 Wochen · pausierbar · reaktivierbar</span>
          </div>
            ...
            {[
              ...
              <div key={p.label} className="bg-white p-6 rounded-sm border border-navy/8">
                ...
                <div className="flex items-baseline gap-1 mb-4">
                  ...
                  <span className="text-xs text-navy/50">/ 4 Wo.</span>
                </div>
                <div className="border-t border-navy/8 pt-3">
                  <p className="text-xs text-navy/55 leading-relaxed">{p.features}</p>
                </div>
              </div>
            ))}
          </div>
          <p className="text-xs text-navy/55 pl-1">+ Werbebudget ab <strong className="text-navy/75">1.400 €</strong> / 4-Wochen-Zyklus (50 €/Tag) — skaliert nach Stellen und Region.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="bg-navy p-7 rounded-sm">
            ...
            <p className="text-white/75 text-base">Vor-Ort-Drehtag: Imagefilm, Fotoreportage, 3 – 5 Mitarbeiterinterviews.</p>
          </div>
          <div className="bg-white p-7 rounded-sm border border-navy/8">
            <p className="text-[10px] tracking-[0.2em] uppercase text-navy/55 mb-3">Vergleich Headhunter</p>
            <p className="font-serif text-2xl text-navy mb-3">12.000 – 18.000 €</p>
            <p className="text-navy/65 text-base">25 – 30 % des Jahresgehalts — pro Steuerfachkraft. Bei mehreren Stellen pro Jahr fünfstelliger Unterschied.</p>
          </div>
        </div>
      </div>
    </section>
